...
export type ClaudeAccountModule = Readonly<{
  ...
  createModel(reference: ClaudeCredentialReference): MastraLanguageModel
  close(): Promise<void>
}>
...
export const createClaudeAccountModule = ({ database, passwordFile, credentialBackend, origin, resolveCurrentSession, fetchImpl }: ClaudeAccountDependencies): ClaudeAccountModule => {
  ...
  return Object.freeze({
    ...
    createModel: (reference) => createAnthropicOAuthModel({ tokenStore: createBackendOAuthTokenStore(
      credentialBackend,
      reference,
      refreshAuthorizationToken,
      {
        ...
        publishRefresh: async ({ current, next, tokens }) => {
          ...
          try {
            await credentialBackend.publishOrMatch(next, plaintext)
          } catch (error) {
