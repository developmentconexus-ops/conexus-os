...
-- The template now runs the agent as an unprivileged user and leaves root to the Hub. The pin an
-- admitted application must carry moves to that build. Only the two literals change; the rest of
-- the function is 0007's, lifted unchanged.
...
BEGIN
  ...
  IF jsonb_typeof(p_payload->'format') IS DISTINCT FROM 'string'
    ...
    OR p_payload->>'templateRef' IS DISTINCT FROM '537fnzf4c16x9d7oz21k:0f44de30-d856-40d1-b6b3-54a8bbf2f440'
    OR jsonb_typeof(p_payload->'recipeSha256') IS DISTINCT FROM 'string'
    OR p_payload->>'recipeSha256' IS DISTINCT FROM 'df2e896284661a4402158d6e694493332df57de4b56f4c565e5b6ed19bfabde4'
