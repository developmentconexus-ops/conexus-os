...
-- The agent sandbox and the compiler sandbox now share one E2B template, so the pin an admitted
-- application must carry moves to the merged build. Only the two literals change; the rest of the
-- function is the baseline's, lifted unchanged.
...
BEGIN
  ...
  IF jsonb_typeof(p_payload->'format') IS DISTINCT FROM 'string'
    ...
    OR p_payload->>'templateRef' IS DISTINCT FROM '537fnzf4c16x9d7oz21k:392ec729-82d7-4f25-bbf0-cc09361611fe'
    OR jsonb_typeof(p_payload->'recipeSha256') IS DISTINCT FROM 'string'
    OR p_payload->>'recipeSha256' IS DISTINCT FROM '6834ca0434e1e6a597340d22d5c1692339a4fc4860948ff834b9851e61406edc'
