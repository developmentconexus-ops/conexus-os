...
import { BaselineCandidate } from '../features/project/components/baseline-candidate'
import { rootRoute } from './__root'

export const projectBaselineCandidateRoute = createRoute({
  getParentRoute: () => rootRoute,
  path: '/projects/$projectId/baseline-candidates/$candidateBaselineDigest',
  component: ProjectBaselineCandidateRoute,
})

function ProjectBaselineCandidateRoute() {
  const { projectId, candidateBaselineDigest } = projectBaselineCandidateRoute.useParams()
  ...
  return (
    <Shell context={access.data} scope={scope}>
      <main><BaselineCandidate key={candidateBaselineDigest} projectId={projectId} candidateBaselineDigest={candidateBaselineDigest} /></main>
    </Shell>
  )
}
