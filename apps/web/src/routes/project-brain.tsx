...
import { ProjectBrain } from '../features/project-resources/components/project-brain'
import { getWorkspace, WorkspaceRequestError, workspaceQueryKey } from '../features/workspace/api'
import { rootRoute } from './__root'

export const projectBrainRoute = createRoute({
  getParentRoute: () => rootRoute,
  path: '/projects/$projectId/brain',
  component: ProjectBrainRoute,
})

function ProjectBrainRoute() {
  const { projectId } = projectBrainRoute.useParams()
  ...
  if (access.isPending || project.isPending || (workspaceId && workspace.isPending)) {
    return <main className="status"><h1>Carregando o Brain do Project</h1></main>
  }
  if (access.isError) {
  ...
  return (
    <Shell context={access.data} scope={{ workspace: workspace.data, project: project.data }}>
      <main><ProjectBrain projectId={projectId} workspaceId={workspace.data.workspaceId} /></main>
    </Shell>
  )
}
