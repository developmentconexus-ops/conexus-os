...
import { WorkspaceBrainSurface } from '../features/brain/components/workspace-brain'
import { accessContextQueryKey, getAccessContext, isAuthenticationRequired } from '../features/identity-access/api'
import { rootRoute } from './__root'

export const workspaceBrainRoute = createRoute({
  getParentRoute: () => rootRoute,
  path: '/workspaces/$workspaceId/brain',
  component: WorkspaceBrainRoute,
})

function WorkspaceBrainRoute() {
  const { workspaceId } = workspaceBrainRoute.useParams()
  ...
  return (
    <Shell context={access.data} scope={{ workspace }}>
      <main className="control-plane-page">
        <nav aria-label="Contexto"><Link to="/" search={{ workspaceId }}>Workspaces</Link> / <span>{workspace.name}</span> / <span>Brain</span></nav>
        <WorkspaceBrainSurface workspaceId={workspaceId} />
      </main>
    </Shell>
  )
}
