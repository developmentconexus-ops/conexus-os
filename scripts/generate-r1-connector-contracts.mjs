...
try {
  ...
  if (process.argv.includes('--check')) {
    if (!existsSync(target) || readFileSync(target, 'utf8') !== output) throw new Error('CONNECTOR_GENERATED_ROUTE_DRIFT')
    if (!existsSync(clientTarget) || readFileSync(clientTarget, 'utf8') !== client) throw new Error('CONNECTOR_GENERATED_CLIENT_DRIFT')
