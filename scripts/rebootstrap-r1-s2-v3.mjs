...
const priorPhases = ['S2-P0', 'S2-P1', 'S2-P2', 'S2-P3', 'S2-P4', 'S2-P5']
const translatedPhases = priorPhases.slice(0, -1)
...
const archive = {
  bundle: 'docs/evidence/4f/s2-p5-reopen-v2/lineage-bundle.json',
  passes: Object.fromEntries(priorPhases.map((part) => [part, `docs/evidence/4f/s2-p5-reopen-v2/${part.slice(3).toLowerCase()}-pass.json`])),
}
const evidencePath = 'docs/evidence/4f/4f-r1-s2-p5-generated-classification-reopen.md'
const translatorPath = 'scripts/rebootstrap-r1-s2-v3.mjs'
...
const expectedArchiveDigests = new Map([
  [archive.passes['S2-P0'], '24fdef8ad89316269d4a5202253f980ed52b1dfe174a36e76e18da1c215f0f59'],
  [archive.passes['S2-P1'], '397862e2c224158bd35027bf1c183af156c1e2366ed9824acf15183c384360d3'],
  [archive.passes['S2-P2'], 'e417b9911c6d98608716b632c4abf3f1bbcfd1e0f7c37488c38475ce55879672'],
  [archive.passes['S2-P3'], '5b2786e1431ff70193dde2c2495a7c06d57633498057a4f2c93a61ffdeb5980a'],
  [archive.passes['S2-P4'], '90e0150a9f1aeab007c1a9fd413cf7866dfd884f82ab22b09557df616779b55c'],
  [archive.passes['S2-P5'], 'cb08957c590541c40c93397a9ad5339ceaa4427b16328b8cfadb24740422ace7'],
  [archive.bundle, 'a377ea37ab93f9944cc3d9ffdf6633304eeadbddf392d368014ad2c80c770f9b'],
])
...
const readLineageBundle = () => {
  ...
  const expectedPaths = [
    ...
    'scripts/record-r1-s2-receipt.mjs',
    'scripts/record-r1-s2-part-pass.mjs',
    'scripts/rebootstrap-r1-s2-v2.mjs',
  ].sort()
  if (bundle.kind !== 'conexus.r1-s2-p5-reopen-v2-archive/v1' ||
...
const validateBootstrapArchive = (priorPlan, entries) => {
  ...
  const priorTranslator = priorPlan.sourceRefs.find(({ path }) => path === 'scripts/rebootstrap-r1-s2-v2.mjs')
  const archivedTranslator = entries.get('scripts/rebootstrap-r1-s2-v2.mjs')
  if (!priorTranslator || !archivedTranslator || priorTranslator.digest !== archivedTranslator.digest) {
    fail('S2_REISSUE_TRANSLATOR_ARCHIVE_DRIFT')
  }
}
...
const validateAdoptedTree = (plan, priorPasses) => {
  ...
  for (const entry of [...plan.changedPaths, ...plan.addedPaths]) {
    ...
    if (latest.get(entry.path) !== entry.currentDigest) fail('S2_REISSUE_LATEST_OWNER_DRIFT', entry.path)
  }
  for (const entry of plan.generatedRoots) {
    ...
    if (latest.get(path) !== entry.currentTreeDigest) fail('S2_REISSUE_LATEST_OWNER_DRIFT', path)
  }
}
...
const sidecar = (path, suffix) => `docs/evidence/4f/s2-p5-reopen-v2/transaction-${basename(path)}${suffix}`
const recoverInterruptedTranslation = ({ checkOnly }) => {
  for (const path of recoveryPaths) {
    const temporary = sidecar(path, '.v3.tmp')
    const backup = sidecar(path, '.v2.bak')
...
const beginTranslation = (values, removals) => {
  ...
  for (const [path, value] of values) durableWrite(sidecar(path, '.v3.tmp'), canonicalBytes(value))
  try {
    for (const path of allPaths) renameSync(absolute(path), absolute(sidecar(path, '.v2.bak')))
    for (const [path] of values) renameSync(absolute(sidecar(path, '.v3.tmp')), absolute(path))
  } catch (error) {
  ...
  return {
    commit: () => {
      for (const path of allPaths) rmSync(absolute(sidecar(path, '.v2.bak')))
    },
    rollback: () => {
      for (const path of allPaths) {
        if (existsSync(absolute(path))) rmSync(absolute(path))
        renameSync(absolute(sidecar(path, '.v2.bak')), absolute(path))
      }
    },
  }
}

export const rebootstrapS2V3 = ({ fableSession, geminiSession, checkOnly = false }) => {
  const uuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/
  ...
  const validation = {
    ...
    reissue: {
      kind: 'conexus.r1-s2-p5-reissue/v1',
  ...
  const removedPath = 'runtime/r1/.conexus/s2-p5-pass.json'
  const transaction = beginTranslation(replacements, [removedPath])
  try {
    ...
    if (existsSync(absolute(removedPath))) fail('S2_REISSUE_P5_REMOVAL_REFUSED')
    for (const part of translatedPhases) {
...
if (isEntrypoint) {
  ...
  process.stdout.write(`${JSON.stringify(rebootstrapS2V3({
