...
const expected003FunctionBodies = (migration, replacementMigration, r2Migration, projectCreateMigration, projectCreatorGrantMigration) => {
  ...
  if (projectCreateMigration) {
    const match = /CREATE OR REPLACE FUNCTION project\.create_project_with_source\([\s\S]*?\nAS \$\$([\s\S]*?)\$\$;/.exec(
      projectCreateMigration.bytes.toString('utf8'),
    )
...
const assert011Catalog = async (
  ...
  await assertSignatures(client, 'MIGRATION_011_CATALOG_REFUSED', `
    ...
    WHERE (table_schema = 'iam' AND table_name IN ('workspace_membership', 'account_project_grant')
      ...
      OR (table_schema = 'reg' AND table_name = 'artifact' AND column_name IN ('published_revision_id'${after026 ? ", 'project_id'" : ''}))
    ORDER BY table_schema, table_name, ordinal_position
  `, [
    ...
    ...(after026 ? ['reg.artifact:project_id:uuid:YES:'] : []),
  ])
  await assertSignatures(client, 'MIGRATION_011_CATALOG_REFUSED', `
    SELECT table_schema || '.' || table_name || ':' || column_name || ':' || udt_name || ':' || is_nullable || ':' ||
      coalesce(column_default, '') AS signature
  ...
  `, [
    ...
    `reg.artifact:workspace_id:uuid:${after026 ? 'YES' : 'NO'}:`,
  ...
  `, [
    ...
    `reg.artifact:artifact_kind_check:c:CHECK (kind ${after026 ? "= ANY (ARRAY['brain'::text, 'application'::text])" : "= 'brain'::text"})`,
    ...(after026 ? [
      `reg.artifact:artifact_kind_ownership_check:c:CHECK (kind = 'brain'::text AND workspace_id IS NOT NULL AND project_id IS NULL OR kind = 'application'::text AND workspace_id IS NULL AND project_id IS NOT NULL AND published_revision_id IS NULL)`,
    ] : []),
    'reg.artifact:artifact_pkey:p:PRIMARY KEY (artifact_id)',
    ...(after026 ? ['reg.artifact:artifact_project_id_fkey:f:FOREIGN KEY (project_id) REFERENCES project.project(project_id) ON DELETE RESTRICT'] : []),
    ...(after026 ? ['reg.artifact:artifact_project_id_kind_key:u:UNIQUE (project_id, kind)'] : []),
  ...
  `, [
    ...
    ...(after026 ? ['reg.artifact:artifact_project_id_kind_key:CREATE UNIQUE INDEX artifact_project_id_kind_key ON reg.artifact USING btree (project_id, kind)'] : []),
...
const assert015Catalog = async (
  ...
  await assertRowsDigest(client, 'MIGRATION_015_CUMULATIVE_FUNCTION_CENSUS_REFUSED', `
    ...
    WHERE n.nspname IN ('iam','workspace','project','reg','brn','con')
      ...
      ${after020 ? "AND NOT (n.nspname = 'iam' AND p.proname = 'admit_project_review')" : ''}
  ...
  await assertSignatures(client, 'MIGRATION_015_TABLE_PRIVILEGE_REFUSED', `
    ...
    allowed(object_name, role_name, privilege_type) AS (VALUES
      ...
      ${after026 ? "('project.project','registry_owner','REFERENCES')," : ''}
      ${after041 ? "('project.project','claude_connection_owner','SELECT')," : ''}
      ${after041 ? "('iam.workspace_membership','claude_connection_owner','SELECT')," : ''}
  ...
  await assertSignatures(client, 'MIGRATION_015_EXTERNAL_PRINCIPAL_ACL_REFUSED', `
    WITH known_principal(role_name) AS (VALUES
      ...
      ${after041 ? "('claude_connection_owner')," : ''}
      ('hub_s2_read'), ('hub_s3_read'), ('hub_s4_baseline_command'), ('hub_s4_baseline_read'),
      ('hub_s6_inception_command'), ('hub_ws01_command'),
      ('brain_owner'), ('connections_owner'), ('iam_owner'), ('project_owner'),
      ('registry_owner'), ('workspace_owner')
  ...
  await assertRowsDigest(client, 'MIGRATION_015_RUNTIME_ACL_CENSUS_REFUSED', `
    ...
    WHERE acl_entries.grantee <> acl_entries.owner_oid
      ...
      ${after026 ? `AND NOT (
        (object_kind = 'schema' AND schema_name = 'project' AND grantee.rolname = 'registry_owner')
        OR (object_kind = 'table' AND schema_name = 'project' AND object_identity = 'project' AND grantee.rolname = 'registry_owner')
        OR (object_kind = 'function' AND schema_name = 'iam' AND object_identity = 'admit_application_build(uuid, uuid)' AND grantee.rolname = 'builder_owner')
        OR (object_kind = 'function' AND schema_name = 'project' AND object_identity = 'lock_application_baseline(uuid)' AND grantee.rolname = 'builder_owner')
        OR (object_kind = 'function' AND schema_name = 'reg' AND object_identity IN (
          'retain_application(uuid, uuid, uuid, text, jsonb)',
          'get_application(uuid, uuid, uuid, text)',
          'read_application_file(uuid, uuid, uuid, text, uuid, text)') AND grantee.rolname = 'hub_rb_executor'))` : ''}
      ${after033 ? `AND NOT (
        (object_kind = 'function' AND schema_name = 'reg' AND object_identity IN (
          'retain_application_execution(uuid, uuid, uuid, text, jsonb)',
          'get_application_execution(uuid, uuid, uuid, text)',
          'read_application_file_execution(uuid, uuid, uuid, text, uuid, text)') AND grantee.rolname = 'hub_rb_executor'))` : ''}
      ${after034 ? `AND NOT (
        ${after038 ? "(object_kind = 'schema' AND schema_name = 'iam' AND grantee.rolname = 'registry_owner') OR" : ''}
        (object_kind = 'function' AND schema_name = 'iam' AND object_identity = 'admit_application_build(uuid, uuid)' AND grantee.rolname = 'registry_owner')
        OR (object_kind = 'function' AND schema_name = 'reg' AND object_identity IN (
          'get_application_by_source(uuid, uuid, text)',
          'read_application_file_by_source(uuid, uuid, text, uuid, text)') AND grantee.rolname = 'hub_rb_executor'))` : ''}
      ${after041 ? `AND NOT (
        (object_kind = 'schema' AND schema_name IN ('iam', 'project') AND grantee.rolname = 'claude_connection_owner')
        OR (object_kind = 'table' AND schema_name = 'iam' AND object_identity = 'workspace_membership' AND grantee.rolname = 'claude_connection_owner')
        OR (object_kind = 'table' AND schema_name = 'project' AND object_identity = 'project' AND grantee.rolname = 'claude_connection_owner'))` : ''}
    ORDER BY object_kind, schema_name, object_identity, grantee.rolname, privilege_type
  `, after026
    ? 'e24dabcff0a51b674aef1572a1e4c5745228b608bb9ce41f2fee5b31e80b6a77'
...
const expected026FunctionBodies = (migration) => {
  const expectedNames = [
    'iam.admit_application_build',
    'project.lock_application_baseline',
    'builder.admit_verified_application_source',
    'reg.retain_application',
    'reg.get_application',
    'reg.read_application_file',
  ]
  const definitions = []
  const pattern = /CREATE FUNCTION (iam|project|builder|reg)\.([a-z0-9_]+)\([\s\S]*?\nAS \$\$([\s\S]*?)\$\$;/g
  for (const match of migration.bytes.toString('utf8').matchAll(pattern)) {
    const name = `${match[1]}.${match[2]}`
    if (expectedNames.includes(name)) definitions.push({ name, definition: match[3] })
  }
  definitions.sort((left, right) => left.name.localeCompare(right.name))
  if (JSON.stringify(definitions.map(({ name }) => name)) !== JSON.stringify([...expectedNames].sort())) {
    fail('MIGRATION_026_FUNCTION_SOURCE_REFUSED')
  }
  return definitions
}

const assert026SchemaPrivileges = async (client) => {
  await assertSignatures(client, 'MIGRATION_026_SCHEMA_PRIVILEGE_REFUSED', `
    SELECT role_name || ':' || schema_name || ':' ||
      has_schema_privilege(role_name, schema_name, 'USAGE') || ':' ||
      has_schema_privilege(role_name, schema_name, 'CREATE') AS signature
    FROM (VALUES
      ('registry_owner', 'builder'), ('registry_owner', 'project'), ('registry_owner', 'reg'),
      ('hub_rb_executor', 'builder'), ('hub_rb_executor', 'project'), ('hub_rb_executor', 'reg'),
      ('public', 'builder'), ('public', 'project'), ('public', 'reg')
    ) AS roles(role_name, schema_name)
    ORDER BY role_name, schema_name
  `, [
    'hub_rb_executor:builder:true:false', 'hub_rb_executor:project:false:false', 'hub_rb_executor:reg:true:false',
    'public:builder:false:false', 'public:project:false:false', 'public:reg:false:false',
    'registry_owner:builder:true:false', 'registry_owner:project:true:false', 'registry_owner:reg:true:true',
  ])
}

const assert026Catalog = async (client, migration026) => {
  const expectedFunctionBodies = expected026FunctionBodies(migration026)
  const functionBodies = (await client.query(`
    SELECT n.nspname || '.' || p.proname AS name, p.prosrc AS definition
    FROM pg_proc AS p JOIN pg_namespace AS n ON n.oid = p.pronamespace
    WHERE (n.nspname, p.proname) IN (
      ('iam', 'admit_application_build'), ('project', 'lock_application_baseline'),
      ('builder', 'admit_verified_application_source'), ('reg', 'retain_application'),
      ('reg', 'get_application'), ('reg', 'read_application_file'))
    ORDER BY name
  `)).rows
  if (JSON.stringify(functionBodies) !== JSON.stringify(expectedFunctionBodies)) {
    fail('MIGRATION_026_FUNCTION_SOURCE_REFUSED')
  }
  await assertSignatures(client, 'MIGRATION_026_FUNCTION_SECURITY_REFUSED', `
    SELECT n.nspname || '.' || p.proname || '(' || oidvectortypes(p.proargtypes) || '):' ||
      pg_get_userbyid(p.proowner) || ':' || p.prosecdef || ':' ||
    ...
    ORDER BY n.nspname, p.proname, oidvectortypes(p.proargtypes)
  `, [
    'builder.admit_verified_application_source(uuid, uuid, uuid, text):builder_owner:true:search_path=pg_catalog, pg_temp:boolean',
    'iam.admit_application_build(uuid, uuid):iam_owner:true:search_path=pg_catalog, pg_temp:TABLE(project_id uuid)',
    'project.lock_application_baseline(uuid):project_owner:true:search_path=pg_catalog, pg_temp:TABLE(baseline_digest text, source_revision text)',
    'reg.get_application(uuid, uuid, uuid, text):registry_owner:true:search_path=pg_catalog, pg_temp:TABLE(artifact_revision_id uuid, artifact_digest text, project_id uuid, source_revision text, profile text, template_ref text, recipe_sha256 text, entry_path text, files jsonb)',
    'reg.read_application_file(uuid, uuid, uuid, text, uuid, text):registry_owner:true:search_path=pg_catalog, pg_temp:TABLE(artifact_revision_id uuid, project_id uuid, source_revision text, path text, media_type text, bytes bytea, sha256 text)',
    'reg.retain_application(uuid, uuid, uuid, text, jsonb):registry_owner:true:search_path=pg_catalog, pg_temp:TABLE(artifact_revision_id uuid, artifact_digest text, project_id uuid, source_revision text, profile text, template_ref text, recipe_sha256 text, entry_path text, files jsonb)',
  ])
  await assertSignatures(client, 'MIGRATION_026_EXECUTE_PRIVILEGE_REFUSED', `
    WITH roles(role_name) AS (
      SELECT rolname FROM pg_roles WHERE rolname LIKE 'hub\\_%' ESCAPE '\\'
      UNION ALL SELECT role_name FROM (VALUES
        ('builder_owner'), ('iam_owner'), ('project_owner'), ('registry_owner'), ('public')
      ) AS owner_roles(role_name)
    ), functions(function_name) AS (VALUES
      ('iam.admit_application_build(uuid, uuid)'),
      ('project.lock_application_baseline(uuid)'),
      ('builder.admit_verified_application_source(uuid, uuid, uuid, text)'),
      ('reg.retain_application(uuid, uuid, uuid, text, jsonb)'),
      ('reg.get_application(uuid, uuid, uuid, text)'),
      ('reg.read_application_file(uuid, uuid, uuid, text, uuid, text)'))
    SELECT roles.role_name || ':' || functions.function_name AS signature
    FROM roles CROSS JOIN functions
    WHERE has_function_privilege(roles.role_name, functions.function_name, 'EXECUTE')
    ORDER BY roles.role_name, functions.function_name
  `, [
    'builder_owner:builder.admit_verified_application_source(uuid, uuid, uuid, text)',
    'builder_owner:iam.admit_application_build(uuid, uuid)',
    'builder_owner:project.lock_application_baseline(uuid)',
    'hub_rb_executor:reg.get_application(uuid, uuid, uuid, text)',
    'hub_rb_executor:reg.read_application_file(uuid, uuid, uuid, text, uuid, text)',
    'hub_rb_executor:reg.retain_application(uuid, uuid, uuid, text, jsonb)',
    'iam_owner:iam.admit_application_build(uuid, uuid)',
    'project_owner:project.lock_application_baseline(uuid)',
    'registry_owner:builder.admit_verified_application_source(uuid, uuid, uuid, text)',
    'registry_owner:reg.get_application(uuid, uuid, uuid, text)',
    'registry_owner:reg.read_application_file(uuid, uuid, uuid, text, uuid, text)',
    'registry_owner:reg.retain_application(uuid, uuid, uuid, text, jsonb)',
  ])
  await assert026SchemaPrivileges(client)
  await assertSignatures(client, 'MIGRATION_026_CROSS_SCHEMA_PRIVILEGE_REFUSED', `
    SELECT object_name || ':' || privilege_type AS signature
    FROM (VALUES
      ('project.project', 'REFERENCES'), ('builder.change', 'SELECT'),
      ('builder.plan', 'SELECT'), ('builder.contract_revision', 'SELECT'),
      ('reg.artifact', 'SELECT'), ('reg.artifact_revision', 'SELECT')
    ) AS required(object_name, privilege_type)
    WHERE (required.object_name = 'project.project' AND NOT has_table_privilege('registry_owner', required.object_name, required.privilege_type))
       OR (required.object_name <> 'project.project' AND has_table_privilege('hub_rb_executor', required.object_name, required.privilege_type))
...
const assert019Catalog = async (client, { after020 = false, after021 = false, after022 = false, after023 = false, after026 = false, after027 = false, after028 = false, after029 = false, after031 = false, after037 = false, after038 = false } = {}) => {
  ...
  await assertSignatures(client, 'MIGRATION_019_TABLE_OWNER_REFUSED', `
    ...
    WHERE n.nspname = 'builder' AND c.relkind = 'r'
      ${after020 ? `AND c.relname NOT IN ('change_acceptance','contract_revision','finding','verification_evidence'${after021 ? ",'finding_resolution'" : ''}${after027 ? ",'project_working_state'" : ''}${after028 ? ",'builder_run'" : ''}${after038 ? ",'actor_run','change','coding_session','operation_receipt','plan','work_unit'" : ''})` : ''}
    ORDER BY relname
  `, after038 ? [] : [
  ...
  `, [])
  if (!after038) await assertSignatures(client, 'MIGRATION_019_WRITER_INDEX_REFUSED', `
    SELECT indexname AS signature FROM pg_indexes
    WHERE schemaname = 'builder' AND indexname = 'one_active_writer_per_change'
      AND indexdef LIKE '${after020 ? "%purpose = ''CODING''%state = ANY (%ADMITTED%RUNNING%" : '%WHERE (state = ANY (%ADMITTED%RUNNING%'}'
  `, ['one_active_writer_per_change'])
}

const assert020Catalog = async (client) => {
  await assertSignatures(client, 'MIGRATION_020_TABLE_OWNER_REFUSED', `
    SELECT relname || ':' || pg_get_userbyid(relowner) AS signature
    FROM pg_class AS c JOIN pg_namespace AS n ON n.oid = c.relnamespace
    WHERE n.nspname = 'builder' AND c.relkind = 'r'
      AND c.relname IN ('change_acceptance','contract_revision','finding','verification_evidence')
    ORDER BY relname
  `, [
    'change_acceptance:builder_owner', 'contract_revision:builder_owner',
    'finding:builder_owner', 'verification_evidence:builder_owner',
  ])
  await assertSignatures(client, 'MIGRATION_020_FUNCTION_SECURITY_REFUSED', `
  ...
  `, [
    ...
    'builder.list_findings:builder_owner:true:search_path=pg_catalog, pg_temp',
    'builder.settle_verification:builder_owner:true:search_path=pg_catalog, pg_temp',
    'iam.admit_project_review:iam_owner:true:search_path=pg_catalog, pg_temp',
  ])
  await assertSignatures(client, 'MIGRATION_020_TABLE_PRIVILEGE_REFUSED', `
    SELECT grantee || ':' || table_name || ':' || privilege_type AS signature
    FROM information_schema.table_privileges
    WHERE table_schema = 'builder' AND grantee IN ('hub_rb_ingress','hub_rb_executor','public')
    ORDER BY grantee, table_name, privilege_type
...
const assert021Catalog = async (client) => {
  await assertSignatures(client, 'MIGRATION_021_TABLE_OWNER_REFUSED', `
    SELECT relname || ':' || pg_get_userbyid(relowner) AS signature
    FROM pg_class AS c JOIN pg_namespace AS n ON n.oid = c.relnamespace
    WHERE n.nspname = 'builder' AND c.relkind = 'r' AND c.relname = 'finding_resolution'
  `, ['finding_resolution:builder_owner'])
  await assertSignatures(client, 'MIGRATION_021_FUNCTION_SECURITY_REFUSED', `
    SELECT p.proname || ':' || pg_get_userbyid(p.proowner) || ':' || p.prosecdef || ':' ||
      coalesce(array_to_string(p.proconfig, ','), '') AS signature
    FROM pg_proc AS p JOIN pg_namespace AS n ON n.oid = p.pronamespace
    WHERE n.nspname = 'builder' AND p.proname IN ('claim_correction','close_finding')
    ORDER BY p.proname
  `, [
    'claim_correction:builder_owner:true:search_path=pg_catalog, pg_temp',
    'close_finding:builder_owner:true:search_path=pg_catalog, pg_temp',
  ])
}
...
const assert023Catalog = async (client) => {
  await assertSignatures(client, 'MIGRATION_023_FUNCTION_SECURITY_REFUSED', `
    SELECT p.proname || ':' || pg_get_userbyid(p.proowner) || ':' || p.prosecdef || ':' ||
      coalesce(array_to_string(p.proconfig, ','), '') AS signature
    FROM pg_proc AS p JOIN pg_namespace AS n ON n.oid = p.pronamespace
...
const assert027Catalog = async (client, { after036 = false } = {}) => {
  ...
  await assertSignatures(client, 'MIGRATION_027_RUNTIME_TABLE_PRIVILEGE_REFUSED', `
    SELECT grantee || ':' || table_name || ':' || privilege_type AS signature
    FROM information_schema.table_privileges
    WHERE table_schema = 'builder' AND grantee IN ('hub_rb_ingress','hub_rb_executor','registry_owner','public'${after036 ? ",'project_owner'" : ''})
      AND table_name = 'project_working_state'
    ORDER BY grantee, table_name, privilege_type
  `, after036 ? ['project_owner:project_working_state:INSERT'] : [])
}

const expected037FunctionBodies = (migration) => {
  const definitions = []
  const pattern = /CREATE (?:OR REPLACE )?FUNCTION (builder\.(?:admit_source_revision|read_latest_code_changing_builder_run))\([\s\S]*?\sAS \$\$([\s\S]*?)\$\$;/g
  for (const match of migration.bytes.toString('utf8').matchAll(pattern)) definitions.push({ name: match[1], definition: match[2] })
  definitions.sort((left, right) => left.name.localeCompare(right.name))
  return definitions
}

const assert037Catalog = async (client, migration037) => {
  const functionBodies = (await client.query(`
    SELECT n.nspname || '.' || p.proname AS name, p.prosrc AS definition
    FROM pg_proc AS p JOIN pg_namespace AS n ON n.oid = p.pronamespace
    WHERE (n.nspname, p.proname) IN (
      ('builder', 'admit_source_revision'), ('builder', 'read_latest_code_changing_builder_run'))
    ORDER BY name
  `)).rows
  if (JSON.stringify(functionBodies) !== JSON.stringify(expected037FunctionBodies(migration037))) fail('MIGRATION_037_FUNCTION_SOURCE_REFUSED')
  await assertSignatures(client, 'MIGRATION_037_FUNCTION_SECURITY_REFUSED', `
    SELECT n.nspname || '.' || p.proname || '(' || oidvectortypes(p.proargtypes) || '):' ||
      pg_get_userbyid(p.proowner) || ':' || p.prosecdef || ':' ||
      coalesce(array_to_string(p.proconfig, ','), '') || ':' || pg_get_function_result(p.oid) AS signature
    FROM pg_proc AS p JOIN pg_namespace AS n ON n.oid = p.pronamespace
    WHERE n.nspname = 'builder'
      AND p.proname IN ('admit_source_revision', 'read_latest_code_changing_builder_run')
    ORDER BY p.proname
  `, [
    'builder.admit_source_revision(uuid, uuid, text):builder_owner:true:search_path=pg_catalog, pg_temp:boolean',
    'builder.read_latest_code_changing_builder_run(uuid, uuid):builder_owner:true:search_path=pg_catalog, pg_temp:jsonb',
  ])
  await assertSignatures(client, 'MIGRATION_037_EXECUTE_PRIVILEGE_REFUSED', `
    WITH functions(name) AS (VALUES
      ('builder.admit_source_revision(uuid, uuid, text)'),
      ('builder.read_latest_code_changing_builder_run(uuid, uuid)'))
    SELECT role_name || ':' || functions.name AS signature
    FROM (VALUES ('hub_rb_ingress'), ('public')) AS roles(role_name)
    CROSS JOIN functions
    WHERE has_function_privilege(roles.role_name, functions.name, 'EXECUTE')
    ORDER BY role_name, functions.name
  `, [
    'hub_rb_ingress:builder.admit_source_revision(uuid, uuid, text)',
    'hub_rb_ingress:builder.read_latest_code_changing_builder_run(uuid, uuid)',
  ])
  await assertSignatures(client, 'MIGRATION_037_TABLE_PRIVILEGE_REFUSED', `
    SELECT grantee || ':' || table_name || ':' || privilege_type AS signature
    FROM information_schema.table_privileges
    WHERE table_schema = 'builder' AND grantee IN ('hub_rb_ingress', 'hub_rb_executor', 'public')
    ORDER BY grantee, table_name, privilege_type
...
const assert038Catalog = async (client, { after040 = false } = {}) => {
  const legacy = (await client.query(`
    SELECT to_regclass('builder.actor_run')::text AS actor_run,
      ...
      to_regclass('builder.finding_resolution')::text AS finding_resolution,
      ...
      to_regprocedure('project.lock_application_baseline(uuid)')::text AS lock_application_baseline,
  ...
  `)).rows[0]
  if (Object.values(legacy).some((value) => value !== null)) fail('MIGRATION_038_LEGACY_CATALOG_REFUSED')
  const currentFunctions = (await client.query(`
    SELECT p.proname
    FROM pg_proc AS p JOIN pg_namespace AS n ON n.oid = p.pronamespace
    WHERE n.nspname = 'builder' AND p.proname IN ('admit_source_revision','read_preview_subject','read_latest_code_changing_builder_run','settle_builder_run_build')
    ORDER BY p.proname
  `)).rows.map(({ proname }) => proname)
...
const verifyLedger = async (client, migrations) => {
  ...
  if (applied.has('022')) {
    ...
    if (applied.has('047')) await assert040Catalog(client)
    else if (applied.has('040')) await assert038Catalog(client, { after040: true })
      ...
      if (applied.has('027')) {
        await assert026SchemaPrivileges(client)
        await assert027Catalog(client, { after036: applied.has('036') })
      }
      if (applied.has('037')) await assert037Catalog(client, files.get('037'))
    }
    ...
    await assert017Catalog(client, files.get('017'))
    await assert018Catalog(client, files.get('018'))
    await assert019Catalog(client, { after020: true, after021: true })
    await assert020Catalog(client)
    await assert021Catalog(client)
  } else if (applied.has('020')) {
    await assert015Catalog(
      client, files.get('003'), files.get('005'), files.get('006'), files.get('011'),
      files.get('012'), files.get('013'), files.get('014'), files.get('015'),
      { after016: true, after017: true, after018: true, after019: true, after020: true },
    ...
    await assert017Catalog(client, files.get('017'))
    await assert018Catalog(client, files.get('018'))
    await assert019Catalog(client, { after020: true })
    await assert020Catalog(client)
  } else if (applied.has('019')) {
    await assert015Catalog(
      client, files.get('003'), files.get('005'), files.get('006'), files.get('011'),
      files.get('012'), files.get('013'), files.get('014'), files.get('015'),
