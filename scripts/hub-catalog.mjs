...
// @mastra/pg creates and migrates the tables inside factory at runtime, as hub_factory, so what
// that schema contains is not the catalog the Hub's migrations produce. The schema line itself,
// with its owner and grants, stays in the catalog: a grant on factory to any other role is drift.
const hubObjects = `${userSchemas} AND n.nspname <> 'factory'`
...
const SECTIONS = Object.freeze({
  ...
  column: `SELECT 'column ' || n.nspname || '.' || c.relname || '.' || a.attname || ' ' || format_type(a.atttypid, a.atttypmod) || ' notnull=' || a.attnotnull || ' default=' || coalesce(pg_get_expr(d.adbin, d.adrelid), '') || ' acl=' || ${effective('a.attacl')} FROM pg_attribute a JOIN pg_class c ON c.oid = a.attrelid JOIN pg_namespace n ON n.oid = c.relnamespace LEFT JOIN pg_attrdef d ON d.adrelid = a.attrelid AND d.adnum = a.attnum WHERE ${hubObjects} AND c.relkind IN ('r', 'p', 'v', 'm', 'f') AND a.attnum > 0 AND NOT a.attisdropped`,
  constraint: `SELECT 'constraint ' || n.nspname || '.' || c.relname || '.' || k.conname || ' ' || pg_get_constraintdef(k.oid) FROM pg_constraint k JOIN pg_class c ON c.oid = k.conrelid JOIN pg_namespace n ON n.oid = c.relnamespace WHERE ${hubObjects}`,
  index: `SELECT 'index ' || n.nspname || '.' || i.relname || ' ' || pg_get_indexdef(i.oid) FROM pg_index x JOIN pg_class i ON i.oid = x.indexrelid JOIN pg_namespace n ON n.oid = i.relnamespace WHERE ${hubObjects}`,
  function: `SELECT 'function ' || n.nspname || '.' || p.proname || '(' || pg_get_function_identity_arguments(p.oid) || ') returns ' || pg_get_function_result(p.oid) || ' kind=' || p.prokind::text || ' owner=' || pg_get_userbyid(p.proowner) || ' secdef=' || p.prosecdef || ' volatility=' || p.provolatile::text || ' config=' || coalesce(array_to_string(p.proconfig, ','), '') || ' acl=' || ${effective("coalesce(p.proacl, acldefault('f', p.proowner))")} || ' body=' || md5(p.prosrc) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace WHERE ${hubObjects}`,
  trigger: `SELECT 'trigger ' || n.nspname || '.' || c.relname || '.' || t.tgname || ' enabled=' || t.tgenabled::text || ' ' || pg_get_triggerdef(t.oid) FROM pg_trigger t JOIN pg_class c ON c.oid = t.tgrelid JOIN pg_namespace n ON n.oid = c.relnamespace WHERE ${hubObjects} AND NOT t.tgisinternal`,
  rule: `SELECT 'rule ' || n.nspname || '.' || c.relname || '.' || r.rulename || ' ' || pg_get_ruledef(r.oid) FROM pg_rewrite r JOIN pg_class c ON c.oid = r.ev_class JOIN pg_namespace n ON n.oid = c.relnamespace WHERE ${hubObjects} AND r.rulename <> '_RETURN'`,
  ...
  type: `SELECT 'type ' || n.nspname || '.' || t.typname || ' kind=' || t.typtype::text || ' owner=' || pg_get_userbyid(t.typowner) || ' labels=' || coalesce((SELECT string_agg(e.enumlabel, ',' ORDER BY e.enumsortorder) FROM pg_enum e WHERE e.enumtypid = t.oid), '') FROM pg_type t JOIN pg_namespace n ON n.oid = t.typnamespace WHERE ${hubObjects} AND t.typtype IN ('e', 'd', 'c') AND NOT EXISTS (SELECT 1 FROM pg_class c WHERE c.reltype = t.oid AND c.relkind <> 'c')`,
  default_acl: `SELECT 'default_acl ' || pg_get_userbyid(d.defaclrole) || ' schema=' || coalesce(n.nspname, '') || ' objtype=' || d.defaclobjtype::text || ' acl=' || ${effective("d.defaclacl")} FROM pg_default_acl d LEFT JOIN pg_namespace n ON n.oid = d.defaclnamespace WHERE n.nspname IS DISTINCT FROM 'factory'`,
