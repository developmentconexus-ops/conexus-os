...
test("refuses literal and server-decoded path escapes", () => {
  for (const locator of [
    "https://git.allowed.test/not-admitted/repo.git",
    "https://git.allowed.test/admitted/..%2f..%2fforbidden/repo.git",
    "https://git.allowed.test/admitted/%252e%252e/%252e%252e/forbidden/repo.git",
    "https://git.allowed.test/admitted/%252f%252fforbidden/repo.git",
    "https://git.allowed.test/admitted/repo.git%00",
    "https://git.allowed.test/admitted/repo.git?upload-pack=/bin/sh",
  ]) {
    assert.deepEqual(admitProviderNeutralLocator(locator), { admitted: false, reason: "DESTINATION_DENIED" });
  }
});
...
for (const projectId of [
  ...
  test(`refuses non-canonical Project path input: ${projectId}`, () => {
    assert.throws(() => resolveCanonicalRepositoryPath("/custody", projectId), /PROJECT_ID_PATH_DENIED/);
  });
}
