...
async function scanForSecret(roots, secretBytes) {
  const scannedFiles = (await Promise.all(roots.map(filesBelow))).flat();
  ...
  return { scannedFiles, contaminatedFiles };
}
function requestFixture(url, ca) {
...
async function main() {
  ...
  try {
    ...
    assert.equal(await requestFixture(`https://git.allowed.test/request-target-negative?canary=${encodeURIComponent(secretCanary)}`, await readFile(certPath)), 404);
  ...
  const rawRequests = JSON.parse(await readFile(requestLogPath, "utf8"));
  const requestTargetNegative = rawRequests.filter(({ requestTarget }) => requestTarget?.includes(secretCanary));
  assert.equal(requestTargetNegative.length, 1);
  const requests = rawRequests.filter(({ requestTarget }) => !requestTarget?.includes(secretCanary));
  await writeFile(requestLogPath, JSON.stringify(requests, null, 2));
  record("REQUEST_TARGET_NEGATIVE", "request-target secret detector fired before retained log scrubbing", { refusalClass: "REQUEST_TARGET_SECRET_DETECTED" });
  assert.equal(requests.some(({ path }) => path.startsWith("/forbidden/")), false); assert.equal(JSON.stringify(requests).includes(secretCanary), false);
  const secretScanRoots = [workRoot, "/root", "/tmp", "/evidence"];
  const secretBytes = Buffer.from(secretCanary); const secretScanNegativePath = join(workRoot, "secret-scan-negative");
  await writeFile(secretScanNegativePath, secretBytes);
  const firedSecretScan = await scanForSecret(secretScanRoots, secretBytes);
  assert.deepEqual(firedSecretScan.contaminatedFiles, [secretScanNegativePath]);
  await unlink(secretScanNegativePath);
  record("SECRET_SCAN_NEGATIVE", "filesystem secret scanner fired against a planted contamination and the owned canary was removed", { refusalClass: "SECRET_SCAN_CONTAMINATION_DETECTED" });
  const { scannedFiles, contaminatedFiles } = await scanForSecret(secretScanRoots, secretBytes);
  ...
  const observedOn = process.env.R1C14_OBSERVED_ON ?? "2026-08-31";
  assert.match(observedOn, /^\d{4}-\d{2}-\d{2}$/);
  const protocolPaths = ["Dockerfile", "README.md", "admission.mjs", "admission.test.mjs", "build-metadata.mjs", "https-fixture.mjs", "probe.mjs", "product-census.mjs", "finalize-result.mjs", "native-readmission-pin.json", "review-brief.md", "run.mjs", "run.ps1", "evidence/native-readmission-rejections/first-vcs-hint-build-metadata.json", "evidence/native-readmission-rejections/first-vcs-hint-rejection.json", "evidence/native-readmission-rejections/second-minimal-provenance-build-metadata.json", "evidence/native-readmission-rejections/second-minimal-provenance-supersession.json", "evidence/superseded-protocol/README.md", "evidence/superseded-protocol/admission-2026-08-31.mjs", "evidence/superseded-protocol/admission.test-2026-08-31.mjs", "evidence/superseded-protocol/finalize-result-2026-08-31.mjs", "evidence/superseded-protocol/https-fixture-2026-08-31.mjs", "evidence/superseded-protocol/probe-2026-08-31.mjs", "evidence/superseded-protocol/product-census-2026-08-31.mjs", "evidence/superseded-protocol/run-2026-08-31.ps1"];
  ...
  const requestTargetSecretAbsent = !JSON.stringify(requests).includes(secretCanary); const secretCanaryAbsent = contaminatedFiles.length === 0;
  const hostNodeVersion = process.env.R1C14_HOST_NODE_VERSION;
  assert.match(hostNodeVersion ?? "", /^v\d+\.\d+\.\d+$/);
  const admissionTestSha256 = process.env.R1C14_ADMISSION_TEST_SHA256;
  assert.match(admissionTestSha256 ?? "", /^[a-f0-9]{64}$/);
  const buildRecipeSha256 = process.env.R1C14_BUILD_RECIPE_SHA256; assert.equal(buildRecipeSha256, protocolDigests.Dockerfile);
  const archiveSha256 = process.env.R1C14_ATTESTED_ARCHIVE_SHA256; assert.match(archiveSha256 ?? "", /^[a-f0-9]{64}$/);
  const signatureSha256 = process.env.R1C14_ATTESTED_SIGNATURE_SHA256; assert.match(signatureSha256 ?? "", /^[a-f0-9]{64}$/);
  const releaseKeySha256 = process.env.R1C14_ATTESTED_RELEASE_KEY_SHA256; assert.match(releaseKeySha256 ?? "", /^[a-f0-9]{64}$/);
  const candidate = { kind: "conexus.r1c14.git-source-custody-results/v2", observedOn, verdict: "PROVISIONAL", predecessors: { s2GenerationReceiptSha256: process.env.R1C14_S2_RECEIPT_SHA256, s2OwnershipManifestSha256: process.env.R1C14_S2_MANIFEST_SHA256 }, provenance: { platform: { os: process.platform, arch: process.arch }, hostRuntime: { node: hostNodeVersion }, baseImage: "node@sha256:be23f54a88d34e8824c741b19b91064094f92c1c97b194144bfc8b50d67258e2", imageIndexDigest: process.env.R1C14_IMAGE_ID, linuxAmd64ManifestDigest: process.env.R1C14_IMAGE_MANIFEST_DIGEST, buildMetadataSha256: process.env.R1C14_BUILD_METADATA_SHA256, buildRecipeSha256, admissionTestSha256, rootfsLayers, rootfsClosureSha256: process.env.R1C14_EXPECTED_ROOTFS_CLOSURE, dependencyClosureSha256: process.env.R1C14_EXPECTED_DEPENDENCY_CLOSURE, executable: { path: git, execPath: gitRun(["--exec-path"]).stdout.trim(), version: gitVersion, sha256: gitSha256, buildFlags, buildOptions }, source: { version: "2.55.0", archiveSha256, signatureSha256, releaseKeySha256, recordedPrimaryKeyFingerprint: "96E07AF25771955980DAD10020D04E5A713660A7", recordedSigningSubkeyFingerprint: "E1F036B1FEE7221FC778ECEFB0B5E88696AFE6CB" }, packages }, protocolDigests, checks, secretScan: { roots: secretScanRoots, scannedFileCount: scannedFiles.length, contaminatedFileCount: contaminatedFiles.length }, cleanup: { temporaryRootRemoved: true, serverStopped, requestTargetSecretAbsent, secretCanaryAbsent } };
