...
"$DOCKER" build \
  ...
  --tag "$IMAGE_TAG" \
  "$SOURCE_ROOT_WIN\\browser-foundation" >/dev/null

IMAGE_ID=$("$DOCKER" image inspect --format '{{.Id}}' "$IMAGE_TAG")
if [ "$IMAGE_ID" != "$EXPECTED_IMAGE_ID" ]; then
  printf 'unadmitted Pack F qualification image\n' >&2
  exit 1
fi

"$DOCKER" run --rm \
  --name conexus-r1f-pack-f \
  --ipc=host \
  --shm-size=1gb \
  --user pwuser \
  --tmpfs /work:rw,exec,mode=1777,size=2147483648 \
  --mount "type=bind,src=$SOURCE_ROOT_WIN,dst=/source,readonly" \
  "$IMAGE_ID" bash /source/gates/pack-f/run-inside.sh
