...
const MUTATIONS = [
  ...
  ['no-storage-source-check', RUN_CLUSTER, ' && line=$(grep -F " /var/lib/conexus-apps-storage " /proc/self/mountinfo | tail -n1) && rest=${line#*" - "} && fstype=${rest%% *} && src=${rest#* } && src=${src%% *} && case $src in /dev/loop*) identity=$(cat "/sys/block/${src#/dev/}/loop/backing_file") ;; *) identity=$src ;; esac && [ "$fstype" = ext4 ] && [ "$identity" = "$CONEXUS_APP_CLUSTER_STORAGE_SOURCE" ]', '', INSTALL],
  ['no-storage-loop-identity-resolution', RUN_CLUSTER, ' && case $src in /dev/loop*) identity=$(cat "/sys/block/${src#/dev/}/loop/backing_file") ;; *) identity=$src ;; esac', ' && identity=$src', INSTALL],
