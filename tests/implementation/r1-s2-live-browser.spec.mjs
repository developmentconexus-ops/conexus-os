...
test('real Keycloak + PostgreSQL + Chromium prove first Workspace, re-entry, idempotency and authority negatives', async ({ browser }) => {
  ...
  await page.getByLabel('Nome do Workspace').fill('<empresa-1>')
  await page.getByRole('button', { name: 'Criar Workspace' }).click()
  await expect(page.getByRole('heading', { name: '<empresa-1>' })).toBeVisible()
  ...
  expect(accessBody.workspaces).toEqual(expect.arrayContaining([expect.objectContaining({ name: '<empresa-1>' })]))
  const primaryWorkspace = accessBody.workspaces.find((workspace) => workspace.name === '<empresa-1>')
  ...
  await expect(page.getByText('<empresa-1>')).toHaveCount(0)
  ...
  await expect(secondaryPage.getByText('<empresa-1>')).toHaveCount(0)
  ...
  const crossWorkspace = await csrfFetch(secondaryPage, { path: `/api/control/workspaces/${primaryWorkspace.workspaceId}` })
  expect(crossWorkspace.status).toBe(404)
  expect(crossWorkspace.body).not.toContain('<empresa-1>')
