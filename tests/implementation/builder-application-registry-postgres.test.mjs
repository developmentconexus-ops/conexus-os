...
test('Registry production adapter retains immutable files through reconnect and enforces admission', async (t) => {
  ...
  await setup.query("INSERT INTO builder.change(change_id, project_id, created_by_account_id, intent, baseline_digest, baseline_source_revision, base_source_revision, planning_depth, rigor_profile, state, candidate_source_revision, patch, result_summary) VALUES ($1, $2, $3, 'Synthetic retention', $4, $5, $5, 'DIRECT', 'CONTROLLED', 'VERIFIED', $6, 'fixture', 'fixture')", [ids.change, ids.project, ids.account, baselineDigest, baseRevision, sourceRevision])
