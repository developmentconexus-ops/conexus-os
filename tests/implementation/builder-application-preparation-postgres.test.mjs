...
test('Builder preparation retains controlled source and reuses it after service recreation', { timeout: 180_000 }, async (t) => {
  ...
  await setup.query('INSERT INTO builder.project_working_state(project_id, working_source_revision) VALUES ($1, $2)', [ids.project, baseRevision])
  await setup.query("INSERT INTO builder.change(change_id, project_id, created_by_account_id, intent, baseline_digest, base_source_revision, baseline_source_revision, planning_depth, rigor_profile, state, candidate_source_revision, patch, result_summary) VALUES ($1, $2, $3, 'Synthetic retention', $4, $5, $5, 'DIRECT', 'CONTROLLED', 'VERIFIED', $6, 'fixture', 'fixture')", [ids.change, ids.project, ids.account, baselineDigest, baseRevision, sourceRevision])
