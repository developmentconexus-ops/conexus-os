...
test('P-01 Build opens the current application with Conexus and creates exact Change work from a Build instruction instead of requiring Change selection', () => {
  ...
  for (const token of [
    ...
    'fixture-only',
    'Current Application',
    'Preview = current Project application by default',
    'Preview + Conexus = default Build entry',
    'Build instruction → BLD-03 CreateChange',
    'Plan instruction → no Change mutation',
    'Change emerges from the work',
    'Build', 'Plan', 'Preview', 'Code', 'Diff',
    'BLD-03', 'BLD-06', 'BLD-07', 'BLD-08', 'BLD-09', 'BLD-10', 'BLD-16',
  ]) requireText(html, token)

  for (const id of [
    'build-root', 'chat-sidebar', 'chat-history', 'new-chat-button',
    'build-composer', 'composer-mode', 'composer-input', 'composer-submit',
    'active-change-context', 'lens-preview', 'lens-code', 'lens-diff',
    'preview-surface', 'source-tree', 'source-file', 'diff-view',
  ]) requireText(html, `id="${id}"`, id)

  for (const behavior of [
    'startNewChat', 'submitComposer', 'createChangeFromInstruction',
...
test('P-01 keeps Plan/progress and Findings/Evidence governed but on demand while the application remains the work center', () => {
  const html = read(htmlPath)
  for (const token of [
    'Visual Plan', 'Hub-owned progress', 'Approve checkpoint', 'Reject checkpoint',
    'Plan current != stale planRevision', 'Findings', 'Evidence',
    'Findings / Evidence = trust layer', 'no Findings != verified', 'Evidence present != claim proven',
  ...
  for (const id of [
    'focus-inspector', 'inspect-plan', 'inspect-findings', 'inspect-evidence', 'inspect-details',
  ...
  for (const behavior of [
    'toggleFocusInspector', 'closeFocusInspector', 'renderPlan', 'decidePlanCheckpoint', 'simulateStalePlan',
    'renderFindings', 'openFinding', 'closeFindingDetail', 'closeFindingWithEvidence', 'renderEvidence',
  ]) requireText(html, behavior, behavior)

  requireText(html, 'planRevision', 'exact Plan revision')
  requireText(html, 'expectedFindingRevision', 'exact Finding revision')
  requireText(html, 'resolutionEvidenceIds', 'Finding closure Evidence')
...
test('P-01 Conexus chat sidebar is the primary Build interaction surface without becoming owner truth or authority', () => {
  ...
  for (const token of [
    ...
    'assistant answer != owner state', 'conversation != Change', 'conversation != Plan truth',
    'conversation != Progress truth', 'conversation != verification',
    'Execution detail', 'execution detail = progressive disclosure', 'BLD-16', 'BLD-17',
    'loading != known-empty != denied != absent/non-disclosable != dependency failure',
    'working != blocked != waiting-for-user != completed', 'Preview ready != verified != live',
  ...
  for (const behavior of ['toggleAssistant', 'submitAssistantQuestion', 'toggleExecutionDetail', 'applyScenario', 'renderScenarioStatus']) {
    requireText(html, behavior, behavior)
  }
...
test('P-01 P8 stays disposable self-contained low-fi Evidence with responsive and accessible interactions', () => {
  ...
  requireText(html, 'P8 WALKTHROUGH FIXTURES', 'fixture legend')
  assert.doesNotMatch(html, /\bfetch\s*\(/, 'P8 must not perform network requests')
  assert.doesNotMatch(html, /XMLHttpRequest|localStorage|sessionStorage|indexedDB/i, 'P8 must not persist or network fixture state')
...
test('P-01 app-first P8 inherits the locked GF-01 Project shell', () => {
  const html = read(htmlPath)
  for (const token of [
    'data-shell="single-adaptive-rail"', 'class="topbar"', 'id="workspaceCrumb"', 'id="projectCrumb"',
    'id="projectRail"', 'Back to Projects', 'Project', 'Product', 'Capabilities', 'Integrations',
    'Operate', 'Activity', 'Manage', 'Settings', 'GF-01 shell inherited',
  ]) requireText(html, token, `locked Project shell token ${token}`)

  const projectRail = html.match(/<section id="projectRail"[\s\S]*?<\/section>/)?.[0] ?? ''
  assert.ok(projectRail, 'P-01 must render Project rail')
  requireText(projectRail, '<button class="rail-btn" type="button" aria-current="page">Build</button>', 'Build current in Project rail')
  ...
  assert.doesNotMatch(projectRail, />Overview<\/button>/, 'no invented Project Overview')
  assert.doesNotMatch(html, /class="global-nav"/, 'no standalone sidebar shell')
})
...
test('P-01 app-first root shows the current application immediately and keeps Change as durable work truth behind chat-driven building', () => {
  const html = read(htmlPath)
  for (const token of [
    'App-first Build root',
    'Current Application',
    'New chat',
    'TODAY',
    'Build / Plan',
    'No active Change',
    'Change becomes active only after a Build instruction',
