...
export { canonicalBytes, sha256 } from '../../canonical-json/src/index.mjs'
...
const visit = (node, text, path = '$') => {
  ...
  if (node.type === 'string') assertPairedSurrogates(node.value, path)
  if (node.type === 'number') {
    ...
    try {
      ...
      fail('INVALID_NUMBER', `invalid number at ${path}`)
    }
...
export const admitJsonBytes = (bytes, validate) => {
  ...
  if (input.length >= 3 && input[0] === 0xef && input[1] === 0xbb && input[2] === 0xbf) fail('BOM_FORBIDDEN', 'UTF-8 BOM is forbidden')
  let text
  try {
  ...
  if (!tree || errors.length) fail('INVALID_JSON', 'strict JSON parse failed')
  visit(tree, text)
  ...
  return Object.freeze({ value, canonical, digest: sha256(canonical) })
}
