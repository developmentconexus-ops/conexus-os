...
- The bytes are persisted. `iam.operation_idempotency` and the other idempotency tables keep `sha256(canonicalBytes(request))`. Any change to the output makes a retried request fail with `IDEMPOTENCY_CONFLICT` after deploy. Treat an output change as a data migration, not a refactor.
