...
These rows are **PROPOSED**: they come from the [single session qualification](../tasks/single-session-qualification.md)
and take effect when the operator accepts its verdict ([evidence](../evidence/single-session/README.md)).
...
| **PROPOSED.** When a due Keycloak check cannot reach Keycloak, every Hub route answers 503 `identity-provider-unavailable`, the Factory's routes included, and the session is kept. | The operator accepted "the session is kept, the Hub answers 503, the Factory answers 401" (D4); the Factory's routes answer 503 instead, because the Hub's own check runs before Mastra's auth. Not decided by the operator. |
