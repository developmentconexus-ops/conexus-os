...
| C-024 | One Conexus installation serves one company and is not multi-tenant. It connects to that company's GitHub organization once, through the Factory's own connect flow, with its own GitHub App installed on all repositories, and every Project becomes a private repository there. The installation is one Factory organization, and Workspaces are not bound to GitHub accounts. Another company runs its own installation with its own App, so no App key ever reaches a second company's code. Decided by the operator on 2026-09-21. | [Factory adoption task](../tasks/factory-adoption.md) | Conexus is asked to serve more than one company from one installation, a Workspace needs its own GitHub account, or GitHub lets an App create repositories in a personal account |
