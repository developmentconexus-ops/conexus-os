...
A Keycloak user deleted and re-created with the same verified email is a new `(issuer, subject)`, because `subject` is Keycloak's, not Conexus's. `iam.provision_application_account` resolves strictly by `(issuer, external_subject)`; a match on email alone never resolves or reactivates the old Account. The old Account and its grants are untouched and keep their history. The new subject reaches Product only through an ordinary invitation to its verified email — the same admission path any other new person uses — and provisions its own distinct Account. Where a live invitation already names that email because the address is still held by the old Account's grant, the new subject gets nothing until an Owner revokes that grant and invites the email again.
