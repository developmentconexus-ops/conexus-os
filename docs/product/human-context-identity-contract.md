...
A Keycloak user deleted and re-created with the same verified email is a new `(issuer, subject)`, because `subject` is Keycloak's, not Conexus's. `iam.provision_application_account` resolves strictly by `(issuer, external_subject)`; a match on email alone never resolves or reactivates the old Account there. `iam.grant_application_access` is different: it looks up open grants by email, so while the old Account's grant for that email is still open, granting the email again answers that same open grant instead of opening an invitation. The old Account and its grants are untouched and keep their history. The new subject reaches Product only once an Owner revokes the old Account's grant and grants the email again — only then does a fresh invitation open, admitting the new subject through the same ordinary path any other new person uses, and provisioning its own distinct Account.

This is why the operator disables a Keycloak user rather than deleting it. Disabling keeps the same `(issuer, subject)` and blocks sign-in outright; the Single-session liveness check independently ends that person's open Hub and application sessions within five minutes regardless of how sign-in was blocked. Deleting instead frees the email for Keycloak to hand to an unrelated new `subject` later, and no migration ever deactivates the old Account on its own — it stays active with its history and any grants an Owner has not revoked. If a Keycloak user is deleted anyway and the email is re-created, the same-email fallback above governs Product/Published-App access. Installation-administrator access is narrower: `iam.grant_installation_administrator_by_email` requires the email to resolve to exactly one active Account and raises `ACCOUNT_EMAIL_AMBIGUOUS` the moment it resolves to more than one, so once both the old and the new Account are active under that email, granting installation-administrator access by email refuses outright rather than picking one.
