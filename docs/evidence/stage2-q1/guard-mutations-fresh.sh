...
# repository's run script on its own small mounted filesystem, and a Hub test cluster
# (conexus-lever-hub, 55463). The two cluster-state mutations each get fresh clusters of their own.
# Needs Docker, passwordless sudo, bubblewrap and the repository's Node; writes guard-mutations.txt
# beside this file.
set -uo pipefail
...
cleanup() {
  ...
  sudo bash "$repository/scripts/mount-application-cluster-storage.sh" remove "$work/apps.img" "$work/apps" >/dev/null 2>&1
  sudo rm -rf "$work/apps"
  rm -rf "$work"
}
trap cleanup EXIT
fresh() {
  ...
  sudo bash "$repository/scripts/mount-application-cluster-storage.sh" remove "$work/apps.img" "$work/apps" >/dev/null 2>&1
  sudo rm -rf "$work/apps"
  sudo bash "$repository/scripts/mount-application-cluster-storage.sh" install "$work/apps.img" 256 "$work/apps" >/dev/null
  bash "$repository/scripts/run-application-cluster.sh" conexus-lever-apps 55462 "$work/apps" "$work/apps-pw" >/dev/null
