...
| 4 | The container refuses to start without that filesystem, on every start | The run script requires the marker, the mountpoint and full allocation before it starts the container; the container's own entrypoint re-checks the marker and, from `/proc/self/mountinfo`, that the mount covering the storage root is still `ext4` from the exact source the run script found, on every start, Docker's restarts included. `application-cluster-installation.test.mjs` proves the restart bypass this closed: a marker plus a stale `pgdata` copied onto the plain directory an unmounted mountpoint leaves behind is refused on start and on restart, run in CI with the runner's own passwordless sudo mounting a throwaway image ([`q1.8/start-guard-proof.txt`](q1.8/start-guard-proof.txt) predates this guard and covers only the marker and mountpoint checks). The pilot's `conexus-apps-postgres` was recreated under the hardened guard (env `CONEXUS_APP_CLUSTER_STORAGE_SOURCE=/dev/loop2`, restart count 0 since recreation); a live Preview read of run-2's note through the certificate relay still passes ([`q1.8/pilot-start-guard.txt`](q1.8/pilot-start-guard.txt) records the prior recreation, also before this guard). |
