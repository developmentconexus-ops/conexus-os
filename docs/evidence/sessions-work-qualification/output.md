...
# store:       /tmp/conexus-qual-NcJHF125
PASS  [write phase] two distinct conversations were created  (7492ab4e vs 884eb7ed)
...
PASS  [thread binding] switch binds the session to the named thread  (7492ab4e)
PASS  [thread binding] switching again rebinds to the other thread  (884eb7ed)
...
PASS  [isolation between Projects] reading another Project's conversation by id is refused  (Thread not found: 7492ab4e)
PASS  [isolation between Projects] binding another Project's conversation by id is refused  (Thread not found: 7492ab4e)
...
# scratch:     /tmp/factory-compat-Uwp87bFI
...
[factory:timing] prepare.storage.init 28ms
[factory:timing] prepare.controllerMount 90ms
[factory:timing] finalize.controller 76ms
...
===== factory work engine =====
PASS  [work item] a work item is created outside the Factory server, against a scratch LibSQL file  (721f88ce rev 1 stage intake)
...
PASS  [evaluation] the accepted move carried the Factory's own decisions rather than the caller's  ([{"type":"sendMessage","idempotencyKey":"factory-stage:f939211a","role":"triage","message":"This work was moved from the intake stag)
PASS  [identity] the stage history names who left intake and who entered triage  ([{"stage":"intake","enteredAt":"2026-09-21T00:05:09.416Z","by":"user-operator","exitedAt":"2026-09-21T00:05:09.422Z","exitedBy":"user-operator"},{"stage":"triage","enteredAt":"2026-09-21T00:05:09.422Z","by":"user-operator"}])
...
PASS  [idempotency] a second request under an ingress identity already seen replays the first answer  (first planning/b320406e, replay planning/b320406e)
...
[factory:timing] prepare.controllerMount 89ms
[factory:timing] finalize.controller 86ms
[factory:timing] finalize.reconcileBoundThreads 0ms
PASS  [project] a Factory project is created with no repository attached  (1f6f3e46 keys 11)
PASS  [conversations under the Factory] the Factory's own controller opens two conversations for a project with no repository and no sandbox  (2d8f5a21 and 2adcecb1)
...
PASS  [conversations under the Factory] the host chose the resourceId, so its own Project identity is what the session is keyed on  (resourceId conexus-project:1f6f3e46)
...
PASS  [conversation] a conversation is created for the Project resource the host chose  (dbda8f0b on conexus-project:alpha)
...
PASS  [source identity] the change is visible to the Project's own git custody as an uncommitted change  (base 44e24697, status "M app/counter.js")
...
PASS  [conversation] a conversation is created on the mounted controller for the host's own Project resource  (312898a5 on conexus-project:alpha)
...
PASS  [the gate] resolving repository access stops without a real App installation  (Version-control repository not found.)

===== the GitHub gate, negative control =====
FAIL  [negative control] repository access resolved without an installation  (this claim is false on purpose)
negative control failed as required

===== a full turn through the code-sdk mount, against a local stub provider =====
# local stub on 127.0.0.1:32857, project at /tmp/code-sdk-turn-qSwDcElv/project
APPROVALS: 1
FINAL_CONTENT: "export const counter = 1\n"
ASSERT_FALSE_MODE: false
PASS
--- the stub's own request log ---
STUB_LISTENING port=32857
...
scratch install kept at /tmp/factory-compat-Uwp87bFI
```
