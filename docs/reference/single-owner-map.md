...
| Model credential storage, selection, visibility and sharing | The Factory, under C-022 and C-025, with its two native levels: just me, and everyone in the installation. | The Hub performs the "everyone" mutation after it checks the installation administrator role. Conexus keeps no grant list and no credential resolver. | Nothing on trunk. The Conexus model subsystem left in migration 0009. |
...
- Model credentials. Factory credentials are stored in plaintext until the Hub configures the
  Factory's `secretEncryption`. Nobody connects a model account before that.
- Model credentials. With `auth: null`, the Factory does not register its per-person credential
  resolver. The per-person path is qualified end to end before anyone is offered a connection.
