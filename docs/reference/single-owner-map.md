...
| Generated application profile and source contract | Conexus owns the supported managed-application profile under C-028; the Project Git repository owns the source bytes. | Factory/Code edits the admitted repository and follows the Project check/Skill; it does not own a second application schema or runtime definition. | The frontend-only assumption grows through the Stage 2 qualifications; no parallel low-code model is added. |
| Application runtime execution | Conexus. Generated handlers execute only through the Conexus application-runtime boundary qualified by Q1. | Factory writes handler source; runtime receives an exact admitted bundle/operation and no model/session authority. | No generated server code may execute inside the Hub merely because the Builder produced it. |
| Project application data | Conexus allocates and authorizes one logical data space per Project/environment; the Project repository owns its schema/migration source. | Factory may edit migration/source files during development but owns no database authority or business-data store. | Mastra/Factory storage is not reused as a second business-data owner. |
| Application audience and runtime grants | Conexus Project policy. App use is independent of Workspace development membership; Keycloak remains the identity door. | Factory receives no app-user roster or runtime grant copy. | The current assumption that every Account may reach Control Plane capabilities is narrowed when Q3 lands. |
| Enterprise Connector definitions, Workspace Connections and Project Grants | Conexus. Connector code/contract is reusable platform authority; a Workspace Connection holds configured access; a Project Grant names admitted operations. | Factory/agents receive only admitted operations/tools when needed, never the underlying provider credential. | The removed connection bindings/capability gateway do not return; Q4 creates the smallest new boundary. |
| Release identity and Publish | Conexus. Release names immutable verified application material; Publish alone selects what employee users receive. | Factory may produce source changes and checks but cannot publish by completing a turn or Work item. | Preview state and Git/PR state never become publication authority. |
...
- Conversation storage. The Factory path is the accepted owner; historical transition wording remains only in closed task evidence.
- Stage 2 application rows describe C-028's accepted ownership direction. Their mechanisms are not called delivered until Q1-Q5 prove them; the roadmap names the only current qualification.
