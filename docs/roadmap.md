...
| 4C — Frontend Interaction & Authority Realization | OPEN / ACTIVE / `GF-01 LOCKED` / `W-01 LOCKED` / `W-02 OPEN` / `F04 GREEN` / `F05 OPERATOR GATE` | Human interactions close with zero invented frontend authority | Material 4A/4B gap or incoherent interaction |
...
```text
4A = CLOSED / N_platform=113 / F04 RECOMPILED
4B = CLOSED / 113↔113 / Project=23 / Connections=9 / F04 RECOMPILED
...
### F04 — Connection identity — ACCEPTED / GREEN

Global Maximum confirmed the existing logical Connection owner and selected `Connection.name` as create-time/server-read human presentation identity, stable across `ConnectionRevision` changes. Rename remains deferred until a real consumer.

Preserved: 113 operations; Connections=9; Permissions=25; `CON-06` configuration-only; no secret readback; `configured != qualified != bound != healthy != authorized`.

Evidence: [finding](evidence/4c/w02-connection-human-identity-finding.md), [Global Maximum](evidence/4c/w02-connection-human-identity-global-maximum.md), [identity authority](product/human-context-identity-contract.md). Proof: #545 GREEN → #546 selected RED → #552 GREEN.

### F05 — Brain Discovery human resolution — OPERATOR GATE
...
Current wire skips the caller-expressible bridge:
...
BRN-05/06 remain proposal reads
BRN-08 remains proposal decision
BRN-09 remains publication
missing property = Discovery-backed proposal intake
leading realization = enrich existing BRN-07; no new owner/operation
```

The leading shape preserves source-backed BRN-07 submission and adds a Discovery-backed path where exact discovery context plus explicit human correction/resolution are revalidated by Brain, which materializes the candidate source revision and returns the same durable `KnowledgeProposal`.

Rejected/deferred: raw Brain-Git revision as ordinary Discovery UX; BRN-04 auto-materialization; Builder/Project-Git reuse; an intermediate resolve operation without its own consumer; `BrainDraft`/`DiscoverySession`/interview-thread domain.

No F05 Product/wire change has been made. Solution-neutral proof #555 is GREEN.
...
If accepted, create selected-realization RED before changing 4A/4B. Do not fabricate the Brain resolution bridge in HTML, lock W-02, open W-03/W-04/P-01, begin 4D, merge PR #57 or implement Product code.
