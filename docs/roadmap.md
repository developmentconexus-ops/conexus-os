...
| 4G — Adversarial Implementation Readiness | NOT STARTED | Fresh independent challenge converges with no unresolved material finding | Material finding falsifies readiness |
| Product implementation | BLOCKED | Requires 4A–4G closed/integrated plus separate explicit operator execution grant | No historical authorization carries forward |
...
**Operator visual/interactive adjudication of the W-01 C1-R1 Baseline HTML candidate: `LOCKED | REVISE | REJECTED`.**
...
→ generated candidate-local selected context
→ contextual Conexus explanation through the locked assistant seam
→ proposed refinements remain non-authoritative local draft
→ explicit Apply refinements boundary
→ new immutable candidate fixture
